...
int main()
{
    ...
    while (exit != 1)
    {
        ...
        while (token)
        {
            ...
            if (strcmp(token, "RR\n") == 0)
                rr_flag = 1;
            if (strcmp(token, "my_kill") == 0)
                my_kill_flag = 1;
            if (strcmp(token, "setmem") == 0)
                setmem_flag = 1;
        ...
        if (mkprocess_flag && token_numeros == 4)
        {
            ...
            {
                printf("Error: El ID '%s' ya existe. Elige un identificador unico.\n", id);
            }
        }
        ...
        if (my_kill_flag && token_numeros == 2)
        {
            ...
            if (memoriaPrincipal)
                freeJSC(memoriaPrincipal, tokens[1]);
            eliminar_proceso(&cabeza, tokens[1]);
        }

        if (setmem_flag && token_numeros == 2)
        {
            int tamano_total = atoi(tokens[1]);
            memoriaPrincipal = crear_bloque(0, tamano_total);

            printf("Memoria principal inicializada.\n");
            printf("Espacio total: %d unidades disponibles.\n", tamano_total);
        }
        ...
        if (alloc_flag && token_numeros == 3)
        {
            ...
            if (enc && memoriaPrincipal)
            {
                ...
                else
                {
                    ...
                    {
                        printf("Error: El proceso %s YA está en memoria (Estado: READY).\n", p->id);
                    }
                    else if (strcmp(p->estado, "TERMINATED") == 0)
                    {
        ...
        if (fcfs_flag)
            fcfsJSC(&cabeza, memoriaPrincipal);
        if (sjf_flag)
            sjfJSC(&cabeza, memoriaPrincipal);
        if (rr_flag)
            rrJSC(&cabeza, 10, memoriaPrincipal);
        if (lstprocss_flag)
            lstprocss(cabeza);
        if (mstatus_flag)
            mstatusJSC(memoriaPrincipal);
        if (compact_flag)
            compactJSC(memoriaPrincipal);
