...
void leer_datos(char *archivo, double *data, char (*clases)[8], int n_features)
{
    ...
    while (leer_linea(&l, linea, BUF_LINEA))
    {
        ...
        for (j = 0; token != NULL && j < n_features; j++)
        {
...
void knnJSC(char entrenamiento[], char prueba[], int k, int hilos)
{
    ...
    if (total_train == 0 || total_test == 0 || nf_train == 0)
    {
    ...
    double accuracy = 0.0;
    if (evaluados > 0)
        accuracy = (double)correctos / evaluados;
    ...
    if (fd_r >= 0)
    {
        ...
        {
            if (test_clases[i][0])
                clase_real = test_clases[i];
            else
                clase_real = "?";
            if (resultados[i])
                clase_pred = resultados[i];
            else
                clase_pred = "?";
